...
def make_breadcrumbs(request, parliament):
    ...
    parliament_num = int(
        request.resolver_match.kwargs.get("parliament_num", CURRENT_PARLIAMENT_NUM)
    )
...
def process_breadcrumbs(breadcrumbs, view):
    ...
    for kwarg in view_kwargs:
        if kwarg == "parliament_num":
            parliament_num = int(
                view_kwargs.get("parliament_num", CURRENT_PARLIAMENT_NUM)
            )
    ...
    if view_name == "day":
        if "input_date" in locals():
            ...
            caption = "%s (%s)" % (
                _("Today's issues"),
                date(requested_date, "SHORT_DATE_FORMAT"),
            )
        else:
            input_date = None
            caption = _("Today's issues")

        breadcrumbs = leave_breadcrumb(breadcrumbs, ("day", input_date), caption)

    if view_name == "upcoming":
        breadcrumbs = leave_breadcrumb(breadcrumbs, ("upcoming",), _("Upcoming Issues"))

    if view_name == "user_issues_monitored":
        breadcrumbs = leave_breadcrumb(
            breadcrumbs, ("user_issues_monitored", parliament_num), _("Monitors")
        )

    if view_name == "user_issues_open":
        breadcrumbs = leave_breadcrumb(
            breadcrumbs, ("user_issues_open", parliament_num), _("Opened Issues")
        )

    if view_name == "user_issues_incoming":
        breadcrumbs = leave_breadcrumb(
            breadcrumbs, ("user_issues_incoming",), _("Issues with new data")
        )
    ...
    if view_name == "parliament_issues":
        breadcrumbs = leave_breadcrumb(
            breadcrumbs, ("parliament_issues", parliament_num), _("Issues")
        )
    ...
    if view_name in ("parliament_committees", "parliament_committee_issues"):
        breadcrumbs = leave_breadcrumb(
            breadcrumbs, ("parliament_committees", parliament_num), _("Committees")
        )

    if view_name in (
        "parliament_committee",
        "parliament_committee_agenda",
        "parliament_committee_issues",
    ):
    ...
    if view_name in ("parliament_categories", "parliament_category"):
        breadcrumbs = leave_breadcrumb(
            breadcrumbs, ("parliament_categories", parliament_num), _("Categories")
        )
    ...
    if view_name == "parliament_persons":
        ...
        breadcrumbs = leave_breadcrumb(
            breadcrumbs,
            (
                ("parliament_persons", parliament_num, party.slug)
                if party
                else ("parliament_persons", parliament_num)
            ),
            caption,
        )
    ...
    if view_name in ("parliament_party", "parliament_party_issues"):
        ...
        breadcrumbs = leave_breadcrumb(
            breadcrumbs, ("parliament_party", parliament_num, party_slug), party
        )
    ...
    if view_name == "person":
        if "subslug" in locals():
            ...
            try:
                person = persons[
                    0
                ]  # We'll only use the name so just need either one of them, doesn't matter which.
            except IndexError:
                person_count = 0
        ...
        if person_count == 1:
            breadcrumbs = leave_breadcrumb(
                ...
                "%s (%s %s)"
                % (person.name, _("b."), date(person.birthdate, "SHORT_DATE_FORMAT")),
            )
        elif person_count > 1:
            breadcrumbs = leave_breadcrumb(
                breadcrumbs, ("person", person.slug), person.name
            )
