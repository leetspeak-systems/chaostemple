        ...
        result.append(
            {
                ...
                "ical": "%s?committee=%s"
                % (
                    request.build_absolute_uri(reverse("ical")),
                    committee.abbreviation_short,
                ),
            }
        )
...
def ical(request):
    ...
    abbreviations = (
        request.GET.get("committee", "").split(",")
    ...
    agendas = (
        ...
        .filter(
            parliament__parliament_num=CURRENT_PARLIAMENT_NUM, committee__in=committees
        )
        .order_by("timing_start_planned")
    )
    ...
    if "issue_nums" in request.GET:
        try:
            ...
            raise AlthingiException(
                'URL parameter "issue_nums" should be a comma-seperated list of integers'
            )
    else:
        issue_nums = None
