    ...
    def handle(self, *args, **options):
        try:
            ...
            while many_found:
                ...
                for com in coms:
                    many_coms = Committee.objects.filter(
                        committee_xml_id=com.committee_xml_id
                    ).order_by("id")
                    if many_coms.count() > 1:
                        ...
                        CommitteeAgenda.objects.filter(
                            committee_id__in=update_to_first
                        ).update(committee_id=first_com.id)
                        Proposer.objects.filter(
                            committee_id__in=update_to_first
                        ).update(committee_id=first_com.id)
