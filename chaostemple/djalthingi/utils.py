...
def maybe_download_document(remote_path, parliament_num, issue_num):
    ...
    local_filename = os.path.join(
        "djalthingi", parliament_num.__str__(), issue_num.__str__(), basename
    )
...
def maybe_download_review(remote_path, log_num, parliament_num, issue_num):
    ...
    local_filename = os.path.join(
        "djalthingi", parliament_num.__str__(), issue_num.__str__(), filename
    )
...
def get_last_parliament_num():
    return (
        althingi_settings.CURRENT_PARLIAMENT_NUM
    )  # Temporary, while we figure out a wholesome way to auto-detect
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        raise ValueError(
                            'Could not figure out datetime format for "%s"' % value
                        )
