...
def content_proxy_view(request, parliament_num, doc_num=None, log_num=None):
    ...
    if doc_num is not None:
        ...
        document = get_object_or_404(
            Document, issue__parliament__parliament_num=parliament_num, doc_num=doc_num
        )
        ...
        review = get_object_or_404(
            Review, issue__parliament__parliament_num=parliament_num, log_num=log_num
        )
    ...
    if pdf_filename:
        filepath = "%s/%s" % (althingi_settings.STATIC_DOCUMENT_DIR, pdf_filename)
