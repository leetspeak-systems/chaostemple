...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        dossiers = issue.dossiers.select_related(
            'review',
            'user'
        ).exclude(
            review=None
        ...
        for dossier in dossiers:
            ...
            if is_duplicate:
                if dossier.is_useful:
                    ...
                    print('Useless duplicate found, marked for deletion: %s, %s' % (
                        dossier.user.username,
                        dossier.review.sender_name
                    ))
        ...
        for dossier in kill_list:
            print(
                'Deleting dossier for %s, %s...' % (dossier.user.username, dossier.review.sender_name),
                end='',
                flush=True
            )
            dossier.delete()
